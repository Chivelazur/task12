...
![Общая схема](uniorb/docs/architecture.drawio.png)
...
- strategy_4 - реализации Стратегии 4 (см. ниже).
...
- Ground - директория с сессиями станций в формате согласно ТЗ.
- Drop - директория с сессиями спутников в формате согласно ТЗ.
- Camera - директория со съемками в формате согласно ТЗ.
...
- satellite_stat.txt - файл со статистикой по спутникам (используется для проверки и отладки). Колонки:
  ...
  - Camera, GB - суммарный объем фактически отснятых данных.
  - Max, GB - теоретический предел объема отснятого материала.
  - Overloads - количество переполнений бортовой памяти.
...
Станция скачивает данные с Киноспутника в первую очередь, если у него более 20% заполненной памяти и он не ведет съемку (берем максимально заполненный из всех удовлетворяющий условию). В остальном - как в Стратегии 1. По сути улучшение Стратегии 2.
20% найдено бинарным поиском.
### Обработка событий
...
Также для оценки решение следует использовать предельные параметры системы:
- Теоретический максимум собранных спутниками данных, если пролетая над РФ в нужное время, спутник всегда ведет съемку: 2245640 GB
- Теоретический максимум полученных на Земле данных, если все станции все время скачивают данные со скоростью передачи КиноСпутника: 14 станций * 13 полных суток * 86400 секунд * 0.125 GB/с = 1965600 GB
...
4. Стратегия 4.
   1. Минут до первого полного заполненния бортовой памяти: 1920
   2. Суммарный объем данных GB, переданных на Землю: 807056
   3. Время работы алгоритма: 1.459 секунд

Стратегии 1 старается максимально уравнять объем занятой бортовой памяти, без учета скорости передачи данных. 

Стратегия 2 интересная тем, что если все время пытаться скачивать данные с Киноспутников, то не даем им заполнить бортовую память достаточно.
...
Резюме:
- Алгоритм крайне прост для понимания, позволяет менять результат путем изменения всего лишь одной стратегии. Наивный вариант показал неплохой результат в ~700k ГБ данных.
- Время работы алгоритма: порядка 1 секунды.
- Рассмотрев все возможные варианты ответвлений (которых порядка триллиона), можно найти самый оптимальный вариант «переключения станций».  Это точное и простое для реализации решение задачи.
- Стратегия позволяет легко:
  - учесть физические ограничения, например, время переключения станции.
  - отдавать приоритет тем или иным спутникам = регионам съемки, что важно для коммерческой эксплуатации системы.
- Скорость алгоритма позволяет параметризовать стратегии и оптимизировать систему градиентным спуском. Например:
  - можно отыскать оптимальное положение новой станции на южном полушарии для увеличения утилизации системы.
  - оптимизировать конфигурацию группировки по типовому составу.
- С++17, без каких-либо сторонних зависимостей, простая архитектура библиотеки.
  
  
__Финальное заключение - Стратегия 4__:
  ...
  - Для оценки возможности улучшения стратегии посчитан теоретический максимум (то есть спутник снимает данные всегда, когда летит над РФ), который можно получить от каждого спутника: 
    - для КиноСпутников коэффициент утилизация = отношение фактически переданных данных к теоретическому максимум, в среднем 94% (абсолютно у всех он больше 91%). Это значит, что утилизиация КиноСпутников крайне высокая. Число переполнений бортовой памяти = 0.
